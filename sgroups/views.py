...
#sys.path.append("/home/lhousehold/smallgroup/sgroups/")

from sgroups.models import Sgroup

# Create your views here.

def index(request):
  template = loader.get_template('sgroups/index.html')
...
def groups(request):
  ...
  for day in days:
    if group_type == 'Women' or group_type == 'Men':
      groups[day] = Sgroup.objects.filter(group_type=group_type,
      day=day) | Sgroup.objects.filter(group_type="Co-ed", day=day)
    else:
      groups[day] = Sgroup.objects.filter(group_type=group_type, day=day)

  template = loader.get_template('sgroups/groups.html')
...
def signup(request):
  ...
  group = Sgroup.objects.get(id=group_id)

  template = loader.get_template('sgroups/signup.html')
  context = RequestContext(request, {"group": group})

  return HttpResponse(template.render(context))

def thankyou(request):
  template = loader.get_template('sgroups/thankyou.html')
  context = RequestContext(request, {})
...
def contact(request):
  ...
  if group.coordinator:
    leader_emails.append(group.coordinator.email)
